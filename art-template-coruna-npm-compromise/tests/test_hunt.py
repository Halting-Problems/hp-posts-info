#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: art-template-coruna-npm-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-art-template-coruna-npm-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-art-template-coruna-npm-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['git.youzzjizz.com', 'v3.jiathis.com', 'utaq.cfww.shop', 'cfww.shop', 'l1ewsu3yjkqeroy.xyz', 'hm.baidu.com'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c (SHA-256, 49554fde7424c31c.js)', '8064d4e0322f069b3dba13e7957ff0ca7dab7984 (SHA-1, 49554fde7424c31c.js)', '6e79ae622b7ef30f31fdbcc2dc65339e (MD5, 49554fde7424c31c.js)', 'f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c', '8064d4e0322f069b3dba13e7957ff0ca7dab7984', '6e79ae622b7ef30f31fdbcc2dc65339e'])
    if not expected:
...
def test_detects_git_youzzjizz_com_in_directory(tmp_path):
    """Script must detect indicator 'git.youzzjizz.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('git.youzzjizz.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'git.youzzjizz.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_v3_jiathis_com_in_directory(tmp_path):
    """Script must detect indicator 'v3.jiathis.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('v3.jiathis.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'v3.jiathis.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_f31bdd069fe7966ae11be1f78ee5dd4444593885_in_directory(tmp_path):
    """Script must detect indicator 'f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c (SHA-256, 49554fde7424c31c.js)' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c (SHA-256, 49554fde7424c31c.js)', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c (SHA-256, 49554fde7424c31c.js)' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_8064d4e0322f069b3dba13e7957ff0ca7dab7984_in_directory(tmp_path):
    """Script must detect indicator '8064d4e0322f069b3dba13e7957ff0ca7dab7984 (SHA-1, 49554fde7424c31c.js)' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('8064d4e0322f069b3dba13e7957ff0ca7dab7984 (SHA-1, 49554fde7424c31c.js)', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '8064d4e0322f069b3dba13e7957ff0ca7dab7984 (SHA-1, 49554fde7424c31c.js)' in hit.txt; "
        f"matches={result['matched']}"
    )
