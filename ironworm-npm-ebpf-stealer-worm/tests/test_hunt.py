#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: ironworm-npm-ebpf-stealer-worm
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-ironworm-npm-ebpf-stealer-worm-scope" in _load_script_source(), (
        "OUT default does not include 'hp-ironworm-npm-ebpf-stealer-worm-scope'"
    )


def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.HASHES constant must match iocs.json iocs.hashes."""
    expected = sorted(['7750bab1a6c48831b5a889e6b799d1684d0a4f2a'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['preinstall hook launching custom Rust binary', 'unusual loading of eBPF programs'])
    if not expected:
...
def test_detects_7750bab1a6c48831b5a889e6b799d1684d0a4f2a_in_directory(tmp_path):
    """Script must detect indicator '7750bab1a6c48831b5a889e6b799d1684d0a4f2a' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('7750bab1a6c48831b5a889e6b799d1684d0a4f2a', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '7750bab1a6c48831b5a889e6b799d1684d0a4f2a' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_ai3_0_3_5_in_directory(tmp_path):
    """Script must detect indicator 'ai3@0.3.5' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('ai3@0.3.5', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'ai3@0.3.5' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_aonote_0_11_1_in_directory(tmp_path):
    """Script must detect indicator 'aonote@0.11.1' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('aonote@0.11.1', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'aonote@0.11.1' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_preinstall_hook_launching_custom_Rust_bi_in_directory(tmp_path):
    """Script must detect indicator 'preinstall hook launching custom Rust binary' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('preinstall hook launching custom Rust binary', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'preinstall hook launching custom Rust binary' in hit.txt; "
        f"matches={result['matched']}"
    )
