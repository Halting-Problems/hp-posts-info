#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: elementary-data-pypi-ghcr-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-elementary-data-pypi-ghcr-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-elementary-data-pypi-ghcr-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['igotnofriendsonlineorirl-imgonnakmslmao.skyhanni.cloud', 'trin.tar.gz'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['elementary-data'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['Python startup executes `elementary.pth`'])
    if not expected:
...
def test_detects_igotnofriendsonlineorirl_imgonnakmslmao_in_directory(tmp_path):
    """Script must detect indicator 'igotnofriendsonlineorirl-imgonnakmslmao.skyhanni.cloud' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('igotnofriendsonlineorirl-imgonnakmslmao.skyhanni.cloud', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'igotnofriendsonlineorirl-imgonnakmslmao.skyhanni.cloud' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_trin_tar_gz_in_directory(tmp_path):
    """Script must detect indicator 'trin.tar.gz' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('trin.tar.gz', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'trin.tar.gz' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_sha256_31ecc5939de6d24cf60c50d4ca26cf7a8_in_directory(tmp_path):
    """Script must detect indicator 'sha256:31ecc5939de6d24cf60c50d4ca26cf7a8c322db82a8ce4bd122ebd89cf634255' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('sha256:31ecc5939de6d24cf60c50d4ca26cf7a8c322db82a8ce4bd122ebd89cf634255', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'sha256:31ecc5939de6d24cf60c50d4ca26cf7a8c322db82a8ce4bd122ebd89cf634255' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_sha256_b3bbfafde1a0db3a4d47e70eb0eb2ca19_in_directory(tmp_path):
    """Script must detect indicator 'sha256:b3bbfafde1a0db3a4d47e70eb0eb2ca19daef4a19410154a71abee567b35d3d9' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('sha256:b3bbfafde1a0db3a4d47e70eb0eb2ca19daef4a19410154a71abee567b35d3d9', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'sha256:b3bbfafde1a0db3a4d47e70eb0eb2ca19daef4a19410154a71abee567b35d3d9' in hit.txt; "
        f"matches={result['matched']}"
    )
