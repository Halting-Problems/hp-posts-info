...
| --- | --- | --- |
| Advisory | `GHSA-5vwr-qchf-q4pf` | Vulnerability identifier for tracking and ticketing. |
| Package version | `@cyclonedx/cdxgen < 12.4.3` | Vulnerable npm package range. |
| Fixed version | `@cyclonedx/cdxgen 12.4.3` | First patched release listed by GHSA. |
| Process pattern | `cdxgen --type maven` | Maven SBOM scan invocation requiring version and input-boundary review. |
| Process pattern | `cdxgen -t maven` | Alternate Maven SBOM scan invocation requiring version and input-boundary review. |
...
Only run downstream credential audits where the exposure table shows vulnerable cdxgen processed untrusted Maven projects. For CI runners, enumerate the exact secrets available to the SBOM job and compare that list to logs from the vulnerable scan window; rotate credentials that were present in the job environment if process telemetry is missing or shows unexpected child processes. For developer workstations, preserve shell history, package manager token files, and source-control credential helper state before rebuilding the toolchain.
