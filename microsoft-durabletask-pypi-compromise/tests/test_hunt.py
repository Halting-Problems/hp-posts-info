#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: microsoft-durabletask-pypi-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-microsoft-durabletask-pypi-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-microsoft-durabletask-pypi-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['www.stepsecurity.io'])
    if not expected:
...
def test_detects_www_stepsecurity_io_in_directory(tmp_path):
    """Script must detect indicator 'www.stepsecurity.io' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('www.stepsecurity.io', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'www.stepsecurity.io' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_durabletask_in_directory(tmp_path):
    """Script must detect indicator 'durabletask' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('durabletask', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'durabletask' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___www_stepsecurity_io_in_directory(tmp_path):
    """Script must detect indicator 'https://www.stepsecurity.io`' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://www.stepsecurity.io`', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://www.stepsecurity.io`' in hit.txt; "
        f"matches={result['matched']}"
    )
