#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: node-gyp-phantom-gyp-npm-worm
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-node-gyp-phantom-gyp-npm-worm-scope" in _load_script_source(), (
        "OUT default does not include 'hp-node-gyp-phantom-gyp-npm-worm-scope'"
    )
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['@vapi-ai/server-sdk@0.7.1', '@vapi-ai/server-sdk@0.7.2', '@vapi-ai/server-sdk@0.7.3', '@vapi-ai/web@2.3.1', '@vapi-ai/web@2.3.2', '@jagreehal/builder@1.0.0', 'abandoned-package@1.0.0', 'abandoned-package-2@1.0.0', 'autotel-terminal@0.0.1', 'autotel-client@0.0.1', 'autotel-trpc@0.0.1'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['node-gyp rebuild', 'npm install lifecycle script', 'native addon build executing during dependency install'])
    if not expected:
...
def test_detects_vapi_ai_server_sdk_0_7_1_in_directory(tmp_path):
    """Script must detect indicator '@vapi-ai/server-sdk@0.7.1' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('@vapi-ai/server-sdk@0.7.1', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '@vapi-ai/server-sdk@0.7.1' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_vapi_ai_server_sdk_0_7_2_in_directory(tmp_path):
    """Script must detect indicator '@vapi-ai/server-sdk@0.7.2' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('@vapi-ai/server-sdk@0.7.2', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '@vapi-ai/server-sdk@0.7.2' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_node_gyp_rebuild_in_directory(tmp_path):
    """Script must detect indicator 'node-gyp rebuild' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('node-gyp rebuild', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'node-gyp rebuild' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_npm_install_lifecycle_script_in_directory(tmp_path):
    """Script must detect indicator 'npm install lifecycle script' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('npm install lifecycle script', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'npm install lifecycle script' in hit.txt; "
        f"matches={result['matched']}"
    )
