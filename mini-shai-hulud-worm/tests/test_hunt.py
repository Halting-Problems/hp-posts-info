#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: mini-shai-hulud-worm
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-mini-shai-hulud-worm-scope" in _load_script_source(), (
        "OUT default does not include 'hp-mini-shai-hulud-worm-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['filev2.getsession.org', 'api.masscan.cloud', 'git-tanstack.com', 't.m-kosche.com', 'www.endorlabs.com', 'www.microsoft.com', 'www.sentinelone.com'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['ab4fcadaec49c03278063dd269ea5eef82d24f2124a8e15d7b90f2fa8601266c'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['@tanstack/react-router', '@tanstack/vue-router', '@tanstack/solid-router', '@tanstack/react-start', '@tanstack/router-core', '@antv/g2', '@antv/g6', '@antv/x6', '@antv/l7', '@antv/s2', '@antv/f2', 'echarts-for-react', 'timeago.js', 'size-sensor', 'canvas-nest.js', '@sap/cds', '@sap/cds-dk', 'opensearch-py', 'lite-llm', 'nx-console'])
    if not expected:
...
def test_detects_filev2_getsession_org_in_directory(tmp_path):
    """Script must detect indicator 'filev2.getsession.org' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('filev2.getsession.org', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'filev2.getsession.org' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_api_masscan_cloud_in_directory(tmp_path):
    """Script must detect indicator 'api.masscan.cloud' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('api.masscan.cloud', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'api.masscan.cloud' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_ab4fcadaec49c03278063dd269ea5eef82d24f21_in_directory(tmp_path):
    """Script must detect indicator 'ab4fcadaec49c03278063dd269ea5eef82d24f2124a8e15d7b90f2fa8601266c' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('ab4fcadaec49c03278063dd269ea5eef82d24f2124a8e15d7b90f2fa8601266c', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'ab4fcadaec49c03278063dd269ea5eef82d24f2124a8e15d7b90f2fa8601266c' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_tanstack_react_router_in_directory(tmp_path):
    """Script must detect indicator '@tanstack/react-router' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('@tanstack/react-router', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '@tanstack/react-router' in hit.txt; "
        f"matches={result['matched']}"
    )
