#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: trivy-pipeline-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-trivy-pipeline-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-trivy-pipeline-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['scan.aquasecurtiy.org', 'www.legitsecurity.com'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['aquasecurity/trivy-action', 'aquasecurity/setup-trivy', 'aquasec/trivy'])
    if not expected:
...
def test_detects_scan_aquasecurtiy_org_in_directory(tmp_path):
    """Script must detect indicator 'scan.aquasecurtiy.org' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('scan.aquasecurtiy.org', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'scan.aquasecurtiy.org' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_www_legitsecurity_com_in_directory(tmp_path):
    """Script must detect indicator 'www.legitsecurity.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('www.legitsecurity.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'www.legitsecurity.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_aquasecurity_trivy_action_in_directory(tmp_path):
    """Script must detect indicator 'aquasecurity/trivy-action' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('aquasecurity/trivy-action', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'aquasecurity/trivy-action' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_aquasecurity_setup_trivy_in_directory(tmp_path):
    """Script must detect indicator 'aquasecurity/setup-trivy' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('aquasecurity/setup-trivy', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'aquasecurity/setup-trivy' in hit.txt; "
        f"matches={result['matched']}"
    )
