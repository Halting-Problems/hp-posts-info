#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: spellcheckpy-typosquatting-rat
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-spellcheckpy-typosquatting-rat-scope" in _load_script_source(), (
        "OUT default does not include 'hp-spellcheckpy-typosquatting-rat-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['www.aikido.dev', 'eu.json.gz'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['spellcheckerpy', 'spellcheckpy'])
    if not expected:
...
def test_detects_www_aikido_dev_in_directory(tmp_path):
    """Script must detect indicator 'www.aikido.dev' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('www.aikido.dev', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'www.aikido.dev' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_eu_json_gz_in_directory(tmp_path):
    """Script must detect indicator 'eu.json.gz' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('eu.json.gz', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'eu.json.gz' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_spellcheckerpy_in_directory(tmp_path):
    """Script must detect indicator 'spellcheckerpy' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('spellcheckerpy', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'spellcheckerpy' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_spellcheckpy_in_directory(tmp_path):
    """Script must detect indicator 'spellcheckpy' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('spellcheckpy', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'spellcheckpy' in hit.txt; "
        f"matches={result['matched']}"
    )
