#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: hades-cluster-pypi-startup-hook-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-hades-cluster-pypi-startup-hook-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-hades-cluster-pypi-startup-hook-compromise-scope'"
    )


def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.HASHES constant must match iocs.json iocs.hashes."""
    expected = sorted(['dc48b09b2a5954f7ff79ab8a2fd80202bd3b59c08c7cdbc6025aa923cb4c0efe', 'e1342a80d4b5e83d2c7c22e1e0aaa95f2d88e3dbf0d853a4994b180c93a4b17d', 'c539766062555d47716f8432e73adbe3a0c0c954a0b6c4005017a668975e275c', '6d332f814f15f19758d65026bbfd0a8c49671b319ec77b8fa1b27fc48afff7d9', '6506d31707a39949f89534bf9705bcf889f1ecae3dbc6f4ff88d67a8be3d01b2'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['Python startup loads a *-setup.pth hook', 'Bun executes _index.js from a Python environment'])
    if not expected:
...
def test_detects_dc48b09b2a5954f7ff79ab8a2fd80202bd3b59c0_in_directory(tmp_path):
    """Script must detect indicator 'dc48b09b2a5954f7ff79ab8a2fd80202bd3b59c08c7cdbc6025aa923cb4c0efe' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('dc48b09b2a5954f7ff79ab8a2fd80202bd3b59c08c7cdbc6025aa923cb4c0efe', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'dc48b09b2a5954f7ff79ab8a2fd80202bd3b59c08c7cdbc6025aa923cb4c0efe' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_e1342a80d4b5e83d2c7c22e1e0aaa95f2d88e3db_in_directory(tmp_path):
    """Script must detect indicator 'e1342a80d4b5e83d2c7c22e1e0aaa95f2d88e3dbf0d853a4994b180c93a4b17d' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('e1342a80d4b5e83d2c7c22e1e0aaa95f2d88e3dbf0d853a4994b180c93a4b17d', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'e1342a80d4b5e83d2c7c22e1e0aaa95f2d88e3dbf0d853a4994b180c93a4b17d' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_Python_startup_loads_a___setup_pth_hook_in_directory(tmp_path):
    """Script must detect indicator 'Python startup loads a *-setup.pth hook' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('Python startup loads a *-setup.pth hook', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'Python startup loads a *-setup.pth hook' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_Bun_executes__index_js_from_a_Python_env_in_directory(tmp_path):
    """Script must detect indicator 'Bun executes _index.js from a Python environment' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('Bun executes _index.js from a Python environment', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'Bun executes _index.js from a Python environment' in hit.txt; "
        f"matches={result['matched']}"
    )
