#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: glassworm-developer-supply-chain-botnet
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-glassworm-developer-supply-chain-botnet-scope" in _load_script_source(), (
        "OUT default does not include 'hp-glassworm-developer-supply-chain-botnet-scope'"
    )


def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.HASHES constant must match iocs.json iocs.hashes."""
    expected = sorted(['1b62b7c2ed7cc296ce821f977ef7b22bae59ef1dcdb9a34ae19467ee39bcf168', '4ebfe8f66ca7e9751060b3301b5e8838d6017593cdae748541de83bfa28183bd', '97c275e3406ad6576529f41604ad138c5bdc4297d195bf61b049e14f6b30adfd'])
    if not expected:
...
def test_detects_1b62b7c2ed7cc296ce821f977ef7b22bae59ef1d_in_directory(tmp_path):
    """Script must detect indicator '1b62b7c2ed7cc296ce821f977ef7b22bae59ef1dcdb9a34ae19467ee39bcf168' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('1b62b7c2ed7cc296ce821f977ef7b22bae59ef1dcdb9a34ae19467ee39bcf168', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '1b62b7c2ed7cc296ce821f977ef7b22bae59ef1dcdb9a34ae19467ee39bcf168' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_4ebfe8f66ca7e9751060b3301b5e8838d6017593_in_directory(tmp_path):
    """Script must detect indicator '4ebfe8f66ca7e9751060b3301b5e8838d6017593cdae748541de83bfa28183bd' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('4ebfe8f66ca7e9751060b3301b5e8838d6017593cdae748541de83bfa28183bd', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '4ebfe8f66ca7e9751060b3301b5e8838d6017593cdae748541de83bfa28183bd' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___nodejs_org_download_release_in_directory(tmp_path):
    """Script must detect indicator 'https://nodejs.org/download/release' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://nodejs.org/download/release', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://nodejs.org/download/release' in hit.txt; "
        f"matches={result['matched']}"
    )
