#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: crypto-key-stealer-typosquats
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-crypto-key-stealer-typosquats-scope" in _load_script_source(), (
        "OUT default does not include 'hp-crypto-key-stealer-typosquats-scope'"
    )


def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.HASHES constant must match iocs.json iocs.hashes."""
    expected = sorted(['d747b41739349828566bfae0b522ef4b746a6f46e828a395d1f3922b66442d40'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['raydium-bs58', 'base-x-64', 'bs58-basic', 'ethersproject-wallet', 'base_xd'])
    if not expected:
...
def test_detects_d747b41739349828566bfae0b522ef4b746a6f46_in_directory(tmp_path):
    """Script must detect indicator 'd747b41739349828566bfae0b522ef4b746a6f46e828a395d1f3922b66442d40' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('d747b41739349828566bfae0b522ef4b746a6f46e828a395d1f3922b66442d40', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'd747b41739349828566bfae0b522ef4b746a6f46e828a395d1f3922b66442d40' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_raydium_bs58_in_directory(tmp_path):
    """Script must detect indicator 'raydium-bs58' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('raydium-bs58', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'raydium-bs58' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_base_x_64_in_directory(tmp_path):
    """Script must detect indicator 'base-x-64' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('base-x-64', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'base-x-64' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___api_telegram_org_bot7231970337_A_in_directory(tmp_path):
    """Script must detect indicator 'https://api.telegram.org/bot7231970337:AAExyV3dvbNs6xkMJB7S2hArUash9owd-bw/sendMessage`' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://api.telegram.org/bot7231970337:AAExyV3dvbNs6xkMJB7S2hArUash9owd-bw/sendMessage`', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://api.telegram.org/bot7231970337:AAExyV3dvbNs6xkMJB7S2hArUash9owd-bw/sendMessage`' in hit.txt; "
        f"matches={result['matched']}"
    )
