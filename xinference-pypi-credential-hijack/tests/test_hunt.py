#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: xinference-pypi-credential-hijack
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-xinference-pypi-credential-hijack-scope" in _load_script_source(), (
        "OUT default does not include 'hp-xinference-pypi-credential-hijack-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['whereisitat.lucyatemysuperbox.space', 'love.tar.gz'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['e1e007ce4eab7774785617179d1c01a9381ae83abfd431aae8dba6f82d3ac127', '077d49fa708f498969d7cdffe701eb64675baaa4968ded9bd97a4936dd56c21c', 'fe17e2ea4012d07d90ecb7793c1b0593a6138d25a9393192263e751660ec3cd0'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['curl --data-binary', 'subprocess.Popen'])
    if not expected:
...
def test_detects_whereisitat_lucyatemysuperbox_space_in_directory(tmp_path):
    """Script must detect indicator 'whereisitat.lucyatemysuperbox.space' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('whereisitat.lucyatemysuperbox.space', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'whereisitat.lucyatemysuperbox.space' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_love_tar_gz_in_directory(tmp_path):
    """Script must detect indicator 'love.tar.gz' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('love.tar.gz', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'love.tar.gz' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_e1e007ce4eab7774785617179d1c01a9381ae83a_in_directory(tmp_path):
    """Script must detect indicator 'e1e007ce4eab7774785617179d1c01a9381ae83abfd431aae8dba6f82d3ac127' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('e1e007ce4eab7774785617179d1c01a9381ae83abfd431aae8dba6f82d3ac127', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'e1e007ce4eab7774785617179d1c01a9381ae83abfd431aae8dba6f82d3ac127' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_077d49fa708f498969d7cdffe701eb64675baaa4_in_directory(tmp_path):
    """Script must detect indicator '077d49fa708f498969d7cdffe701eb64675baaa4968ded9bd97a4936dd56c21c' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('077d49fa708f498969d7cdffe701eb64675baaa4968ded9bd97a4936dd56c21c', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '077d49fa708f498969d7cdffe701eb64675baaa4968ded9bd97a4936dd56c21c' in hit.txt; "
        f"matches={result['matched']}"
    )
