#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: gemstuffer-rubygems-exfiltration-channel
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-gemstuffer-rubygems-exfiltration-channel-scope" in _load_script_source(), (
        "OUT default does not include 'hp-gemstuffer-rubygems-exfiltration-channel-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['rubygems.org', 'moderngov.lambeth.gov.uk', 'democracy.wandsworth.gov.uk', 'moderngov.southwark.gov.uk'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['239440c830e17530dda0a8a06ed2708860998750a1e3ed2239e919465dc59420', 'c2d6bcacc88177e0f2c8c262726f86f37e671b1692c8bc135bac4b610ddcf31a', '34212b88108cab6ded037257d6fbc79a61b4c2ea8ecddc6c513b5aad1f308638', '2e4e099275efb8f886824a8eccdc595e624cd08ebb1772bd427710e08ff3ab24', '94d6c0b589704c8cc75e19f7250d6bfda473266dd7dd7e23fd14bd1bb972a717'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['ruby writing /tmp/gemhome/.gem/credentials', 'ruby running gem build', 'ruby running gem push', 'ruby Net::HTTP::Post to RubyGems'])
    if not expected:
...
def test_detects_rubygems_org_in_directory(tmp_path):
    """Script must detect indicator 'rubygems.org' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('rubygems.org', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'rubygems.org' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_moderngov_lambeth_gov_uk_in_directory(tmp_path):
    """Script must detect indicator 'moderngov.lambeth.gov.uk' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('moderngov.lambeth.gov.uk', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'moderngov.lambeth.gov.uk' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_239440c830e17530dda0a8a06ed2708860998750_in_directory(tmp_path):
    """Script must detect indicator '239440c830e17530dda0a8a06ed2708860998750a1e3ed2239e919465dc59420' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('239440c830e17530dda0a8a06ed2708860998750a1e3ed2239e919465dc59420', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '239440c830e17530dda0a8a06ed2708860998750a1e3ed2239e919465dc59420' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_c2d6bcacc88177e0f2c8c262726f86f37e671b16_in_directory(tmp_path):
    """Script must detect indicator 'c2d6bcacc88177e0f2c8c262726f86f37e671b1692c8bc135bac4b610ddcf31a' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('c2d6bcacc88177e0f2c8c262726f86f37e671b1692c8bc135bac4b610ddcf31a', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'c2d6bcacc88177e0f2c8c262726f86f37e671b1692c8bc135bac4b610ddcf31a' in hit.txt; "
        f"matches={result['matched']}"
    )
