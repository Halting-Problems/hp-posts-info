#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: packagist-github-postinstall-hook-campaign
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-packagist-github-postinstall-hook-campaign-scope" in _load_script_source(), (
        "OUT default does not include 'hp-packagist-github-postinstall-hook-campaign-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['github.com'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['moritz-sauer-13/silverstripe-cms-theme', 'crosiersource/crosierlib-base', 'devdojo/wave', 'devdojo/genesis', 'katanaui/katana', 'elitedevsquad/sidecar-laravel', 'r2luna/brain', 'baskarcm/tzi-chat-ui'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['curl -skL ... -o /tmp/.sshd', 'chmod +x /tmp/.sshd', '/tmp/.sshd running in background'])
    if not expected:
...
def test_detects_github_com_in_directory(tmp_path):
    """Script must detect indicator 'github.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('github.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'github.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_moritz_sauer_13_silverstripe_cms_theme_in_directory(tmp_path):
    """Script must detect indicator 'moritz-sauer-13/silverstripe-cms-theme' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('moritz-sauer-13/silverstripe-cms-theme', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'moritz-sauer-13/silverstripe-cms-theme' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_crosiersource_crosierlib_base_in_directory(tmp_path):
    """Script must detect indicator 'crosiersource/crosierlib-base' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('crosiersource/crosierlib-base', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'crosiersource/crosierlib-base' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_curl__skL______o__tmp__sshd_in_directory(tmp_path):
    """Script must detect indicator 'curl -skL ... -o /tmp/.sshd' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('curl -skL ... -o /tmp/.sshd', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'curl -skL ... -o /tmp/.sshd' in hit.txt; "
        f"matches={result['matched']}"
    )
