#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: intercom-client-npm-shai-hulud
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-intercom-client-npm-shai-hulud-scope" in _load_script_source(), (
        "OUT default does not include 'hp-intercom-client-npm-shai-hulud-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['api.github.com', 'metadata.google.internal'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['5f748fbc89cde66abefa826439c765a0081a027792e9da8d80fbf23571311622', 'fe64699649591948d6f960705caac86fe99600bf76e3eae29b4517705a58f0e2', '5ae8b2343e97cc3b2c945ec34318b63f27fa2db1e3d8fbaa78c298aa63db52ed'])
    if not expected:
...
def test_script_ips_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['169.254.169.254'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['intercom-client'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['npm preinstall launches Bun-backed loader files'])
    if not expected:
...
def test_detects_metadata_google_internal_in_directory(tmp_path):
    """Script must detect indicator 'metadata.google.internal' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('metadata.google.internal', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'metadata.google.internal' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_5f748fbc89cde66abefa826439c765a0081a0277_in_directory(tmp_path):
    """Script must detect indicator '5f748fbc89cde66abefa826439c765a0081a027792e9da8d80fbf23571311622' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('5f748fbc89cde66abefa826439c765a0081a027792e9da8d80fbf23571311622', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '5f748fbc89cde66abefa826439c765a0081a027792e9da8d80fbf23571311622' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_fe64699649591948d6f960705caac86fe99600bf_in_directory(tmp_path):
    """Script must detect indicator 'fe64699649591948d6f960705caac86fe99600bf76e3eae29b4517705a58f0e2' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('fe64699649591948d6f960705caac86fe99600bf76e3eae29b4517705a58f0e2', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'fe64699649591948d6f960705caac86fe99600bf76e3eae29b4517705a58f0e2' in hit.txt; "
        f"matches={result['matched']}"
    )
