#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: node-ipc-expired-domain-takeover
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-node-ipc-expired-domain-takeover-scope" in _load_script_source(), (
        "OUT default does not include 'hp-node-ipc-expired-domain-takeover-scope'"
    )
...
def test_detects_node_ipc_in_directory(tmp_path):
    """Script must detect indicator 'node-ipc' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('node-ipc', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'node-ipc' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___snyk_io_in_directory(tmp_path):
    """Script must detect indicator 'https://snyk.io`' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://snyk.io`', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://snyk.io`' in hit.txt; "
        f"matches={result['matched']}"
    )
