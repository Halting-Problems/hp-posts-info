#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: laravel-lang-composer-tag-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-laravel-lang-composer-tag-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-laravel-lang-composer-tag-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['flipboxstudio.info'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['2f0ee073c6f29d66188a845592029c9b52528f04'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['laravel-lang/lang', 'laravel-lang/http-statuses', 'laravel-lang/actions', 'laravel-lang/attributes'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['php -r require vendor/autoload.php followed by orphaned php', 'sh -c php /tmp/.laravel_locale/.php > /dev/null 2>&1 &', 'nohup /tmp/'])
    if not expected:
...
def test_detects_flipboxstudio_info_in_directory(tmp_path):
    """Script must detect indicator 'flipboxstudio.info' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('flipboxstudio.info', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'flipboxstudio.info' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_2f0ee073c6f29d66188a845592029c9b52528f04_in_directory(tmp_path):
    """Script must detect indicator '2f0ee073c6f29d66188a845592029c9b52528f04' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('2f0ee073c6f29d66188a845592029c9b52528f04', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '2f0ee073c6f29d66188a845592029c9b52528f04' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_laravel_lang_lang_in_directory(tmp_path):
    """Script must detect indicator 'laravel-lang/lang' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('laravel-lang/lang', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'laravel-lang/lang' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_laravel_lang_http_statuses_in_directory(tmp_path):
    """Script must detect indicator 'laravel-lang/http-statuses' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('laravel-lang/http-statuses', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'laravel-lang/http-statuses' in hit.txt; "
        f"matches={result['matched']}"
    )
