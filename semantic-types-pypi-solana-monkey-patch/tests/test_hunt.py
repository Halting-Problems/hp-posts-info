#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: semantic-types-pypi-solana-monkey-patch
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-semantic-types-pypi-solana-monkey-patch-scope" in _load_script_source(), (
        "OUT default does not include 'hp-semantic-types-pypi-solana-monkey-patch-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['api.devnet.solana.com', 'solders.keypair.Keypair'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['5a4d8480c9d1e82ba102f200258882fb9e694e8fc0343b6982c5540beccdca62'])
    if not expected:
...
def test_detects_api_devnet_solana_com_in_directory(tmp_path):
    """Script must detect indicator 'api.devnet.solana.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('api.devnet.solana.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'api.devnet.solana.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_solders_keypair_Keypair_in_directory(tmp_path):
    """Script must detect indicator 'solders.keypair.Keypair' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('solders.keypair.Keypair', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'solders.keypair.Keypair' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_5a4d8480c9d1e82ba102f200258882fb9e694e8f_in_directory(tmp_path):
    """Script must detect indicator '5a4d8480c9d1e82ba102f200258882fb9e694e8fc0343b6982c5540beccdca62' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('5a4d8480c9d1e82ba102f200258882fb9e694e8fc0343b6982c5540beccdca62', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '5a4d8480c9d1e82ba102f200258882fb9e694e8fc0343b6982c5540beccdca62' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___api_devnet_solana_com_in_directory(tmp_path):
    """Script must detect indicator 'https://api.devnet.solana.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://api.devnet.solana.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://api.devnet.solana.com' in hit.txt; "
        f"matches={result['matched']}"
    )
