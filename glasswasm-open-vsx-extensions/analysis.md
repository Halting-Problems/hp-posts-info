...
The following selectors are directly supported by the reviewed source material and by current registry-state checks [1][2][3].
...
### Publisher and Hosting Selectors
...
The first downstream audit is the developer-host secret boundary. On any host with a positive match, review whether the affected IDE session or the same machine held GitHub credentials, npm tokens, cloud SDK credentials, SSH keys, container-registry logins, or blockchain wallet material. This is not speculative boilerplate; it follows directly from Socket's confirmed `execSync`-based download-and-execute design and the unrecovered second stage [1].

The second downstream audit is the registry and identity boundary. Socket says the clean VS Code Marketplace originals remained intact, so audit any internal allowlists or mirror rules that trust an extension name across registries without binding it to a registry-specific publisher identity [1]. That control gap is what made these identity-cloned Open VSX listings plausible [1].
