#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: tanstack-pipeline-poisoning
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-tanstack-pipeline-poisoning-scope" in _load_script_source(), (
        "OUT default does not include 'hp-tanstack-pipeline-poisoning-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['git-tanstack.com'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['@tanstack/zod-adapter', '@tanstack/router', '@tanstack/react-router', '@tanstack/react-query', '@tanstack/table-core'])
    if not expected:
...
def test_detects_git_tanstack_com_in_directory(tmp_path):
    """Script must detect indicator 'git-tanstack.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('git-tanstack.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'git-tanstack.com' in hit.txt; "
...
def test_detects_tanstack_zod_adapter_in_directory(tmp_path):
    """Script must detect indicator '@tanstack/zod-adapter' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('@tanstack/zod-adapter', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '@tanstack/zod-adapter' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_tanstack_router_in_directory(tmp_path):
    """Script must detect indicator '@tanstack/router' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('@tanstack/router', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '@tanstack/router' in hit.txt; "
        f"matches={result['matched']}"
    )
