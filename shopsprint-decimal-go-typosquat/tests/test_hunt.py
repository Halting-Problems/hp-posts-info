#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: shopsprint-decimal-go-typosquat
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-shopsprint-decimal-go-typosquat-scope" in _load_script_source(), (
        "OUT default does not include 'hp-shopsprint-decimal-go-typosquat-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['dnslog-cdn-images.freemyip.com', 'freemyip.com'])
    if not expected:
...
def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c'])
    if not expected:
...
def test_script_packages_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['github.com/shopsprint/decimal'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['Go application importing github.com/shopsprint/decimal'])
    if not expected:
...
def test_detects_dnslog_cdn_images_freemyip_com_in_directory(tmp_path):
    """Script must detect indicator 'dnslog-cdn-images.freemyip.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('dnslog-cdn-images.freemyip.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'dnslog-cdn-images.freemyip.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_freemyip_com_in_directory(tmp_path):
    """Script must detect indicator 'freemyip.com' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('freemyip.com', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'freemyip.com' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_f31bdd069fe7966ae11be1f78ee5dd4444593885_in_directory(tmp_path):
    """Script must detect indicator 'f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'f31bdd069fe7966ae11be1f78ee5dd44445938856dd1df12379e0e84a6851f5c' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_github_com_shopsprint_decimal_in_directory(tmp_path):
    """Script must detect indicator 'github.com/shopsprint/decimal' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('github.com/shopsprint/decimal', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'github.com/shopsprint/decimal' in hit.txt; "
        f"matches={result['matched']}"
    )
