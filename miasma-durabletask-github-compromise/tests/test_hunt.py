#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: miasma-durabletask-github-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-miasma-durabletask-github-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-miasma-durabletask-github-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['api.masscan.cloud', 'filev2.getsession.org', 'seed1.getsession.org', 'seed2.getsession.org', 'seed3.getsession.org', 'git-tanstack.com'])
    if not expected:
...
def test_detects_https___api_masscan_cloud_in_directory(tmp_path):
    """Script must detect indicator 'https://api.masscan.cloud' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://api.masscan.cloud', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://api.masscan.cloud' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_vscode_tasks_json_in_directory(tmp_path):
    """Script must detect indicator '.vscode/tasks.json' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('.vscode/tasks.json', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '.vscode/tasks.json' in hit.txt; "
        f"matches={result['matched']}"
    )
