#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: chromium-browser-fetch-leak-zero-day
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-chromium-browser-fetch-leak-zero-day-scope" in _load_script_source(), (
        "OUT default does not include 'hp-chromium-browser-fetch-leak-zero-day-scope'"
    )


def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.HASHES constant must match iocs.json iocs.hashes."""
    expected = sorted(['2976e22e8416cf3a341294855047bb6280ced6b2'])
    if not expected:
...
def test_detects_2976e22e8416cf3a341294855047bb6280ced6b2_in_directory(tmp_path):
    """Script must detect indicator '2976e22e8416cf3a341294855047bb6280ced6b2' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('2976e22e8416cf3a341294855047bb6280ced6b2', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '2976e22e8416cf3a341294855047bb6280ced6b2' in hit.txt; "
