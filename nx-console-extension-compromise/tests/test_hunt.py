#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: nx-console-extension-compromise
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-nx-console-extension-compromise-scope" in _load_script_source(), (
        "OUT default does not include 'hp-nx-console-extension-compromise-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['sfrclak.com', 'com.user.kitty-monitor.plist'])
    if not expected:
...
def test_detects_com_user_kitty_monitor_plist_in_directory(tmp_path):
    """Script must detect indicator 'com.user.kitty-monitor.plist' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('com.user.kitty-monitor.plist', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'com.user.kitty-monitor.plist' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_nx_console_in_directory(tmp_path):
    """Script must detect indicator 'nx-console' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('nx-console', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'nx-console' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___sfrclak_com_api_v1_beacon_in_directory(tmp_path):
    """Script must detect indicator 'https://sfrclak.com/api/v1/beacon' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://sfrclak.com/api/v1/beacon', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://sfrclak.com/api/v1/beacon' in hit.txt; "
        f"matches={result['matched']}"
    )
