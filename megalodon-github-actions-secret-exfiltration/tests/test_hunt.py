#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: megalodon-github-actions-secret-exfiltration
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-megalodon-github-actions-secret-exfiltration-scope" in _load_script_source(), (
        "OUT default does not include 'hp-megalodon-github-actions-secret-exfiltration-scope'"
    )


def test_script_hashes_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.HASHES constant must match iocs.json iocs.hashes."""
    expected = sorted(['1c9e803c80cc7fed000022d4c94f4b5bc2e90062', '7f6120bb10c870b9fde146961a18e5bf0b3d4401', 'acac5a9854650c4ae2883c4740bf87d34120c038'])
    if not expected:
...
def test_script_ips_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['216.126.225.129'])
    if not expected:
...
def test_script_process_patterns_constant_matches_iocs(script_tree, iocs):
    ...
    expected = sorted(['workflow collects environment variables and credential files'])
    if not expected:
...
def test_detects_1c9e803c80cc7fed000022d4c94f4b5bc2e90062_in_directory(tmp_path):
    """Script must detect indicator '1c9e803c80cc7fed000022d4c94f4b5bc2e90062' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('1c9e803c80cc7fed000022d4c94f4b5bc2e90062', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '1c9e803c80cc7fed000022d4c94f4b5bc2e90062' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_7f6120bb10c870b9fde146961a18e5bf0b3d4401_in_directory(tmp_path):
    """Script must detect indicator '7f6120bb10c870b9fde146961a18e5bf0b3d4401' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('7f6120bb10c870b9fde146961a18e5bf0b3d4401', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '7f6120bb10c870b9fde146961a18e5bf0b3d4401' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_216_126_225_129_in_directory(tmp_path):
    """Script must detect indicator '216.126.225.129' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('216.126.225.129', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator '216.126.225.129' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_workflow_collects_environment_variables_in_directory(tmp_path):
    """Script must detect indicator 'workflow collects environment variables and credential files' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('workflow collects environment variables and credential files', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'workflow collects environment variables and credential files' in hit.txt; "
        f"matches={result['matched']}"
    )
