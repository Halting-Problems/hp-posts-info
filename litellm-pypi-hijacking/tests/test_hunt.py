#!/usr/bin/env python3
"""
Auto-generated per-folder tests for: litellm-pypi-hijacking
...
def test_script_out_default_is_namespaced():
    ...
    assert "hp-litellm-pypi-hijacking-scope" in _load_script_source(), (
        "OUT default does not include 'hp-litellm-pypi-hijacking-scope'"
    )


def test_script_domains_constant_matches_iocs(script_tree, iocs):
    """SCRIPT.DOMAINS constant must match iocs.json iocs.domains."""
    expected = sorted(['www.litellm.ai'])
    if not expected:
...
def test_detects_www_litellm_ai_in_directory(tmp_path):
    """Script must detect indicator 'www.litellm.ai' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('www.litellm.ai', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'www.litellm.ai' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_litellm_in_directory(tmp_path):
    """Script must detect indicator 'litellm' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('litellm', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'litellm' in hit.txt; "
        f"matches={result['matched']}"
    )

def test_detects_https___www_litellm_ai_in_directory(tmp_path):
    """Script must detect indicator 'https://www.litellm.ai`' when present in a scanned file."""
    scan_dir = tmp_path / 'scan'
    scan_dir.mkdir()
    out_dir = tmp_path / 'out'
    out_dir.mkdir()
    hit_file = scan_dir / 'hit.txt'
    hit_file.write_text('https://www.litellm.ai`', encoding='utf-8', errors='replace')
    result = _run_scan_in_proc(scan_dir, out_dir)
    assert len(result['matched']) >= 1, (
        f"Script failed to detect indicator 'https://www.litellm.ai`' in hit.txt; "
        f"matches={result['matched']}"
    )
