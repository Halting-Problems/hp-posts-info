...
def test_ioc_contract():
 d=json.loads((ROOT/'iocs.json').read_text()); assert all('@' not in x for x in d['affected_assets']['packages']); assert all(isinstance(x,str) for x in d['iocs']['package_versions'])
